...
category.add = function(data,results){
    db.query("INSERT INTO category (Name, Image) VALUES (?, ?)", [data.Name, data.Image], function (err,categories) {
        if (err) return err
        else {
...
category.find = function(data,results){
    ...
    {
        db.query("SELECT * FROM category WHERE id = ?",data.id, function (err,categories) {
            if(err) return err
            else{
...
category.delete = function(idCategory,results){
    db.query("DELETE FROM category WHERE id =?",idCategory, function (err,categories) {
        if (err) return err
        else {
            console.log(categories)
