...
rating.add = function(data,results){
    db.query("INSERT INTO rating (id_Book,id_Account, star,comment) VALUES (?, ?, ?, ?)", [data.id_Book, data.id_Account,data.star,data.commemt], function (err,ratings) {
        if (err) return err
        else {
...
rating.findOne = function(data,results){
    console.log(data)
    db.query("SELECT * FROM rating WHERE id_Book = ? AND id_Account = ?",[data.idBook,data.idAccount], function (err,rating) {
        if(err) return err
        else{
            results(rating)
        }
...
rating.update = function(idRating, data,results){
    db.query("UPDATE rating SET star =?, comment = ? WHERE id =?", [data.star, data.comment, idRating], function (err,rating) {
        if (err) return err
        else {
