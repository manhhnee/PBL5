...
class AuthController {
    ...
    isAdmin(req, res, next) {
        if (req.user.role === 'ADMIN') {
            ...
            return res.status(401).json({success:false, message:'yêu cầu đăng nhập là admin'});
        }
    }
    isStaff(req, res, next) {
        if (req.user.role === 'STAFF') {
            ...
            return res.status(401).json({success:false, message:'yêu cầu đăng nhập là staff'});
        }
    }
    isCustomer(req, res, next) {
        if (req.user.role === 'CUSTOMMER') {
            ...
            return res.status(401).json({success:false, message:'yêu cầu đăng nhập là customer'});
        }
    }
}
